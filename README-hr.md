...
- **前端闭环（2026-08-16）**：新增 `/hr/search` 简历语义检索独立菜单页（自然语言/模式/TopK/元信息展示），AI 设置支持 Rerank 模型，候选人简历弹窗展示索引状态/流转日志/删除，审计日志补齐 SEARCH/OFFER/HANDOFF/IMPORT/MERGE 标签；`vue-tsc`、admin/chat 双构建通过。
- **前端 HR 优先改造（2026-08-16）**：根路由与登录后落地到 `/hr/candidates`，顶栏仅保留「人事部」菜单（Agent/知识库/模型菜单隐藏、URL 直连仍可用，系统设置走头像菜单），移除 Upgrade 商业版入口；新增 HrBrand 品牌组件（顶栏/登录页/关于弹窗），登录 slogan 与页面标题改为「人事招聘工作台」；修复 `/admin/locales/index.json` 404。HR 页面：共享常量抽至 `src/views/hr/constants.ts`（标签映射/状态机常量/格式化），候选人页操作列收纳「更多」下拉并分组页头按钮，职位页展开区升级为**管道看板**（待筛选→筛选通过→面试中→Offer 中→已入职+已结束 6 列，拖拽仅允许沿链前进、终态/回退弹原因对话框并校验，卡片菜单含编辑指派/淘汰/退出/关闭/恢复）；顺带修复展开职位后候选人标签页默认不激活的既有缺陷；`vue-tsc`、eslint、admin 构建通过，无头浏览器验证拖拽流转/非法回拖拦截/终态收纳。
