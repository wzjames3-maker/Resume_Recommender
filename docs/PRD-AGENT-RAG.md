...
> 文档状态：设计基线，2026-08-16。本文件是 `docs/PRD.md` 的增补案：裁决 §6「AI、知识库与集成边界」的 Agent 化扩展，并在 §9.2 路线表后增补 **D 阶段（Agent 智能化）**。它不推翻 §2.2「人工决策优先」，而是给出该原则的工程化形态（§2）。本文件已随 PRD 同版本纳入 D 阶段路线图；后续变更须同步更新 PRD 与本文件。
>
> 输入依据：① 项目现状勘察（RAG 检索链路与评测、权限审计已交付；Agent 层为零；**ATS v2 目标设计见 `docs/ATS-STATE-MACHINE-V2.md`，传统 ATS 开源调研见 `docs/ATS-OPENSOURCE-REFERENCE.md`，均为 D 阶段前置**）；② 传统 ATS 的 AI Copilot 原则（Harly：context before verbosity / evidence before confidence / propose-confirm-execute / backend-enforced safety）；③ 外部参考架构《Agent + RAG 智能 ATS 系统设计》（propose-approve、证据引用、双表示简历、中心化编排四原则被吸收，技术选型按本项目裁决，见 §13）。

## 0. 结论先行

子系统 = **已有的 RAG 检索服务 + 重构后的 ATS v2 状态机（命令 + 领域事件）** 之间补一层 **Agent 运行时**（`apps/hr/agents/`）：Agent 由事件/人工触发，经白名单工具调用既有检索服务，产出**带证据的提议工件（HrAgentProposal）**，人审批后调用 ATS v2 命令层执行。不引入新框架、新运行时、新向量库；不新增对外通信通道。首期（D1）只做一个 Agent：**初筛评估（Screening）**，因为它 100% 复用已交付的检索服务；待办队列按 `ATS-DESIGN-SPEC.md` §6 契约补齐后作为建议落点，直接兑现 C 阶段 RAG 投入（recall@5=0.92 目前只在「搜人」一个触点产生价值）。
...
| # | 问题 | 事实依据 |
|---|---|---|
| 1 | **AI 角色停留在「检索框」**：AI 只出现在自然语言搜人、技能抽取、语义检索三个触点，评估报告、建议动作、反馈闭环均未定义。RAG 已达标（RRF+rerank recall@5=0.92）但只在「搜人」发光，投入产出不成比例。 | PRD §5.2、§6；`apps/hr/services/resume_search.py` 已交付 |
| 2 | **「人工决策优先」没有被工程化**：原则写了「AI 只提供建议」，但建议无数据形态、无审批对象、无决策记录——散落在报告里无法审计、无法度量采纳率、无法迭代。 | PRD §2.2；代码中无任何 proposal/建议工件 |
| 3 | **无 Agent 运行账本**：现有 `ResumeFlowLog` 只覆盖简历入库六节点；`ai_parser.py` 三个 LLM 调用是隐形调用，无统一输入/工具链/成本/失败记录，**不满足 PRD §6 自己提出的**「外部 LLM 调用记录租户、模型、发送数据类别、用途、失败情况」。 | `apps/hr/services/ai_parser.py` |
| 4 | **企业知识库资产未接入**：MaxKB 本职就是企业知识库（JD 模板、面试题库、政策、FAQ），工作区知识库现成可用；参考架构中 JD 对标、面试挂题库、答疑挂 FAQ 在本项目本是零成本优势，PRD 的 AI 边界里只有简历一个语料源。 | PRD §6；`apps/knowledge` 内核完好 |
| 5 | **路线图断头**：§9.2 到 C 阶段为止，C 的完成定义已达成（2026-08-16），后续无方向。 | PRD §9.2 |
...
2. **提议-审批（propose-approve），即「人工决策优先」的工程化形态**：Agent 的唯一写出口是 `HrAgentProposal` 工件；一切状态变更由人审批后调 **ATS v2 命令层**执行（操作者=审批人，沿用 ApplicationEvent 时间线与 ASSIGNMENT_TRANSITION 审计）。零风险产出（报告、草稿、标签）免审批；有风险动作（婉拒、推进）默认必审；D3 才评估对「低风险+高一致性」分带开放可配置免审，且保留审计与人工回滚。
3. **双表示简历**：结构化字段（`Candidate`/`CandidateSkill`，SQL 精确过滤硬条件）+ 语义分块（既有简历语义索引，软条件匹配）——已实现，直接复用，不重复建设。
4. **中心化编排**：不引入 LangGraph/多 Agent 自由协作。Agent = Django 服务 + Celery 任务 + 显式触发器；流程锚定在 ATS v2 的 `CandidateAssignment` 状态机与 `AssignmentEvent` 上，可控、可审计、可重放（`HrAgentRun` 全量留痕）。
...
- **触发器**（三类）：
  - 事件：`CandidateAssignment` 创建/状态迁移的信号 → Celery 异步（不阻塞业务请求）；
  - 定时：celery-beat（如面试前 Copilot 提醒，D2）；
  - 人工：REST 端点显式触发（`POST /workspace/{ws}/hr/agents/{type}/run`，OPERATOR 起）。
...
| `search_resumes(query, filters, candidate_id=None, document_ids=None)` | `resume_search` 服务 | 读 | 复用双路召回→RRF→rerank→Small-to-Big；D1 仅在召回入口新增**可选文档集限定**，默认行为不变 |
...
|---|---|---|---|---|---|---|
| **Screening 初筛评估** | **D1** | 新建 `ACTIVE` Application（投递/内推，初始 Stage=`APPLIED`） | 人岗匹配评估报告 + 建议动作（§6） | get_job, get_candidate_overview, structured_filter, search_resumes, propose | 建议必审 | 待办队列/候选人详情 |
...
| 沟通草稿助手 | D3 | 阶段流转/人工触发 | 拒信/进度通知/答疑话术草稿（FAQ 挂企业知识库） | search_knowledge, propose | 草稿免审；**外发永远人工** | 候选人详情/关联时间线 |
...
**数据边界重申（继承 PRD §1/§6）**：候选人数据永不写入企业知识库；`search_knowledge` 只读；简历语义索引仅存可重建的脱敏派生数据。前置整改：简历语义索引当前以普通知识库形态暴露在全局知识库列表、可被删除（当前代码事实，D1 前整改），D1 前须改为受人事模块管理的受保护索引。

**对 RAG 链路的最小改动承诺**：D1 只给 `search_resumes` 增加可选 `candidate_id/document_ids` 召回范围参数；查询理解、双路召回、RRF、rerank、Small-to-Big、降级链与评测口径均不变。未传该参数时行为与 C 阶段交付完全一致。
...
|---|---|
| run FK | 溯源到运行账本 |
| target_type / target_id | ASSIGNMENT / JOB / INTERVIEW |
...
- **审计**：AGENT_RUN / AGENT_DECIDE 入 HrAuditLog（只增不改）；建议借此补齐 trace 字段（现存缺口，Agent 链路放大其必要性：run ↔ 业务操作关联追溯）。
...
| 页面 | 改动 |
|---|---|
| 待办/工作台 | 关联列表增加 Agent 建议徽标（ADVANCE/HOLD/DECLINE + score）与筛选 |
| 候选人详情 | **AI 初筛报告卡**：维度结论 + 证据摘录（点击回溯简历原文定位）+ concerns/澄清问题 + 一键确认/婉拒/忽略；操作走既有敏感操作确认弹窗（含审计提示） |
| 职位编辑（D2） | JD 草稿对比采纳（草稿 ≠ 写入） |
| my-interviews（D2） | 面试问题清单/评估草稿，手动提交为反馈 |
| audit | AGENT_RUN/AGENT_DECIDE 动作过滤 |
...
| 2 | HrAuditLog 无 trace 字段（PRD §7 已标未实现） | D1 一并补齐，run ↔ 审计关联依赖它 |
| 3 | 简历语义索引暴露于全局知识库列表、可被删除 | D1 前整改为受保护索引（§7） |
