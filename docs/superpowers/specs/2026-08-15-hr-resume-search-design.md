...
**命中判定（排序与放宽的基准）**：技能 s 命中简历 r ⇔ s 在 r 的任意段落中 dense similarity ≥ **相对阈值** = max(绝对阈值 0.2, 该技能召回最高分 × 0.75)。**实测依据（2026-08-16）**：短技能词（java/fastapi/rag）对任意段落的 dense 相似度天然落在 0.2~0.42 低区分度区间，固定阈值 0.2 下人人命中、命中向量失去区分度——必须用该技能召回最高分的 75% 作相对阈值收窄（仍以 0.2 为下限兜底）。命中判定只看 dense 路分数（尺度稳定可比），RRF/rerank 不参与命中判定，只用于同 hit_vec 内二次排序与最终精排。
