...
> **修复状态（2026-08-16，内核 P1 修复轮）：P1×3 已全部修复**——K1 Fork 抓取 SSRF 加固（8638956，超时 5s/20s + 私网/元数据黑名单含重定向）；K2 Web 文档同步网络抓取移出事务（6fc68c5）；K3 聊天异常文本不回传用户（99292fa）。P2/P3 未动（见 §3/§4 清单，按需立项）。
