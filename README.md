...
- **L3 证据层**：`RRF(k=60)` 免调参融合 → `bge-reranker-v2-m3` 精排 → 简历级证据合成 `max(段分) + λ·log(1+命中段数)`，每份命中简历输出 top-3 证据段落（title+分数），HR 可逐条核验。
- **L4 呈现与合规**：简历卡片 = 结构化字段（`_mask_for_role` 角色掩码）+ 证据段引用 + 原文行号回溯高亮。

### 数据面：切片协议与三副本索引

简历解析时**一次 LLM 调用**同时产出章节边界与结构化字段（PII 掩码前置，行号协议不受影响）；每份简历切成 `{title, section_type, 行号区间}` 的段落（500 字上限，同章节小段自动合并），每段同时落三份索引：
