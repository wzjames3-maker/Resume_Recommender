...
- ✅ **内核 P1 修复（2026-08-16，3 提交，基线 392/392）**：K1 Fork 抓取 SSRF 加固（8638956：超时 5s/20s + 私网/云元数据黑名单含重定向跳转，阻断内网探测；4 新测试）；K2 Web 同步网络抓取移出事务（6fc68c5：慢站点不再长占 DB 连接/锁，成功/失败路径行为保持，3 新测试）；K3 聊天异常文本不回传用户（99292fa：异常细节仅入日志，回答与 chat_record 为通用文案，1 新测试）。P2/P3 未立项。
